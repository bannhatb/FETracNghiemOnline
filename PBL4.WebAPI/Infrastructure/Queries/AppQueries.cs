...
namespace PBL4.WebAPI.Infrastructure.Queries
{
    public class AppQueries : IAppQueries
    {
        ...
        public async Task<QuestionDetailQueryModel> GetQuestionDetailById(int questionId)
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append(@" SELECT q.Id, q.QuestionContent, q.Explaint, q.RightCount, q.TypeId 
...
INNER JOIN Questions as q on qc.QuestionId = q.Id where q.Id = @questionId ");
        ...
        public async Task<List<QuestionQueryModel>> GetListQuestionInExam(int examId, UrlQuery urlQuery)
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append(@" SELECT q.Id, q.QuestionContent, q.Explaint, q.TypeId, q.RightCount 
...
");
                var result = await connection
                    .QueryAsync<QuestionQueryModel>(sb.ToString(), new { examId = examId,
                        page = urlQuery.Page,
...
");
                var result = await connection
                    .QueryAsync<TestQuestionQueryModel>(sb.ToString(), new { userId = userId, testId = testId,
                    page = urlQuery.Page,
                    pageSize = urlQuery.PageSize});
                return result.ToList();
            }
...
");
        ...
        public async Task<ExamDetailQueryModel> GetExamDetailById(int examId)
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append(@" SELECT e.Id, e.Title, e.Time, e.CreateBy, e.QuestionCount, e.LevelId,  e.IsPublic
...
INNER JOIN Exams as e on ec.ExamId = e.Id where e.Id = @examId ");
        ...
        public async Task<LevelQueryModel> GetNameLevelExam(int examId)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                StringBuilder sb = new StringBuilder();
                sb.Append(@" SELECT l.LevelName 
FROM Levels as l INNER JOIN Exams as e on e.LevelId = l.Id 
where e.Id = @examId ");
                var result = await connection
